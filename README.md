...
- [kamilcuk/nomad-tools](https://github.com/Kamilcuk/nomad-tools) - Collection of the following tools:
    - `nomadtools go` - Just like `docker run`, generate a Nomad job specification from command line arguments and "watch" over the execution of that job.
    - `nomadtools watch` - Run Nomad jobs from the terminal while collecting all tasks logs and allocations events and then exit with the task exit status.
    - `nomadtools cp` - Copy files between host and Nomad allocations just like `docker cp`.
    - `nomadtools vardir` - Set, get and list [Nomad variables](https://developer.hashicorp.com/nomad/docs/concepts/variables) one variable key at a time.
    - `nomadtools dockers` - List docker images referenced by a Nomad job or Nomad job file.
    - `nomadtools port` - List ports allocated to Nomad allocation similar to `docker port`.
    - `nomadtools gitlab-runner` - Implementation of [custom `gitlab-runner` executor](https://docs.gitlab.com/runner/executors/custom.html) that executes Gitlab CI/CD jobs as Nomad jobs.
    - `nomadtools downloadrelease` - Download specific Hashicorp compiled binary version from Hashicorp releases page.
